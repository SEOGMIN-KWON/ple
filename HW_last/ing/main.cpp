...
int main(int argc, char **argv) { 
	...
	for(i = 0; i < num_threads; i++) {	
		int ret = pthread_create(&thread[i], NULL, &worker, (void *)ht);
		if (ret) {
...
int main(int argc, char* argv[]){
	...
	/* clock() function : measure the execution time (s) */
	clock_t init_tick = clock(); // time unit : Clock Tick
	double result_time;
	...
	/* convert the time: Clock Tick => seconds AND save the result_time */
	result_time = (double) (clock() - init_tick) / CLOCKS_PER_SEC;
	printf(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n");
