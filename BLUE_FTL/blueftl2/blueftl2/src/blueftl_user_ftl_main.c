...
// 		res += d[i]*d[i];
// 	}
// 	return (res/1024.0) - avg*avg;
// }

void print_block_erasures(int sig){ // can be called asynchronously
...
int32_t blueftl_user_ftl_create (struct ssd_params_t* ptr_ssd_params)
{
	...
	act_new.sa_handler = print_block_erasures; // 시그널 핸들러 지정
			...
			for (lpa_curr = lpa_begin; lpa_curr < lpa_end; lpa_curr++) {
				...
				if (_ftl_base.ftl_get_mapped_physical_page_address (
					...
					blueftl_user_vdevice_page_read (
					   ...
					   (char*)ptr_lba_buff);
					// if(block==0 &&page==0) printf("%d %d rd %x\n", block, page, ptr_lba_buff);
				} else {
			...
			for (lpa_curr = lpa_begin; lpa_curr < lpa_end; lpa_curr++) {
				...
				if (is_merge_needed == 0) {
					...
					blueftl_user_vdevice_page_write (
						...
						_ptr_vdevice->page_main_size, 
						(char*)ptr_lba_buff);
					// if(block ==0 && page==0) printf("%d %d wt %x\n", block, page, ptr_lba_buff);
					// if(lpa_curr >= 65535) printf("lpa_curr %d \n",lpa_curr);
