...
void __check_(struct ftl_context_t* ptr_ftl_context){
   int i,b=0;
   for(i=0;i<1024;i++){
      if(ptr_ftl_context->ptr_ssd->list_buses[0].list_chips[0].list_blocks[i].list_pages > (struct flash_block_t*)0xf00000000000){
        if(b==0){printf("error");}
        b=1;
        printf("%d",i);}
    }
	...
	)
{
	...
	if (old_ppa!=-1){// && (new_ppa != old_ppa)) {
		...
		*/
		uint32_t old_chunk_index = convert_ppa_to_chunk_index(old_ppa);
		ptr_chunk_table[old_chunk_index].valid_count--;
		// printf("decrease valid count\n");
		//valid한 page가 없을 때, 압축된 모든 페이지를 invalidation
		if(ptr_chunk_table[old_chunk_index].valid_count==0){
			...
			for(i=0; i<ptr_chunk_table[old_chunk_index].physical_page_len; i++){
...
int32_t page_mapping_map_logical_to_physical (
	...
	uint32_t nr_logical_pages, uint32_t nr_physical_pages, bool is_compressed)
{
	struct ftl_page_mapping_context_t* ptr_pg_mapping = (struct ftl_page_mapping_context_t*)(ptr_ftl_context->ptr_mapping);
	struct chunk_table_t *ptr_chunk_table = ptr_pg_mapping->ptr_chunk_table;
	struct flash_ssd_t *ptr_ssd = ptr_ftl_context->ptr_ssd;
	struct flash_block_t *flash_block = &(ptr_ftl_context->ptr_ssd->list_buses[bus].list_chips[chip].list_blocks[block]);
	uint32_t ppa = ftl_convert_to_physical_page_address (bus, chip, block, page);
	uint32_t chunk_index = convert_ppa_to_chunk_index(ppa);
	uint32_t i;

	if(flash_block->nr_valid_pages==0 && flash_block->nr_invalid_pages==0){
		ptr_ssd->list_buses[bus].list_chips[chip].nr_free_blocks--;
		ptr_ssd->list_buses[bus].list_chips[chip].nr_dirty_blocks++;
	}
	...
	**/
	...
	uint32_t b=0;
	// for(i=0; i<nr_logical_pages; i++){
	// 	if(ptr_pg_mapping->ptr_pg_table[logical_page_address[i]] != -1 &&
	// 		ptr_pg_mapping->ptr_pg_table[logical_page_address[i]] != ftl_convert_to_physical_page_address (bus, chip, block, page))
	// 	{
	// 	// if(logical_page_address[i]%1024 == 0 &&ptr_pg_mapping->ptr_pg_table[logical_page_address[i]] != -1){
	// 		if(b==0) printf("start\n");
	// 		b=1;
	// 		printf("lpa %d, compressed %d\n", logical_page_address[i], is_compressed);
	// 		printf("old ppa:chunk_index %d:%d\n", ptr_pg_mapping->ptr_pg_table[logical_page_address[i]], convert_ppa_to_chunk_index(ptr_pg_mapping->ptr_pg_table[logical_page_address[i]]));
	// 		printf(", validcount %d %p\n", ptr_chunk_table[convert_ppa_to_chunk_index(ptr_pg_mapping->ptr_pg_table[logical_page_address[i]])].valid_count, &(ptr_chunk_table[convert_ppa_to_chunk_index(ptr_pg_mapping->ptr_pg_table[logical_page_address[i]])].valid_count));
	// 		printf("new ppa:chunk_index %d:%d\n", ppa, convert_ppa_to_chunk_index(ppa));
			
	// 	}
	// 	else if(logical_page_address[i] < 2){
	// 		printf("- lpa\n");
	// 		printf("lpa %d, compressed %d\n", logical_page_address[i], is_compressed);
	// 		if(ptr_pg_mapping->ptr_pg_table[logical_page_address[i]] != -1)
	// 			printf("old ppa:chunk_index %d:%d\n", ptr_pg_mapping->ptr_pg_table[logical_page_address[i]], convert_ppa_to_chunk_index(ptr_pg_mapping->ptr_pg_table[logical_page_address[i]]));
	...
	if(is_compressed){
		for(i=0; i<WRITE_BUFFER_LEN; i++){
			// if(ptr_pg_mapping->ptr_pg_table[logical_page_address[i]] != -1)
			// 	printf("[validcount %d %p]\n", ptr_chunk_table[convert_ppa_to_chunk_index(ptr_pg_mapping->ptr_pg_table[logical_page_address[i]])].valid_count, &(ptr_chunk_table[convert_ppa_to_chunk_index(ptr_pg_mapping->ptr_pg_table[logical_page_address[i]])].valid_count));
			// printf("params %d %d %d\n", logical_page_address[i], block, page+i);
		...
		// n개의 페이지에 대한 pg table 업데이트 
		for(i=0; i<nr_logical_pages; i++){
			ptr_pg_mapping->ptr_pg_table[logical_page_address[i]] = ppa;
		}

		// m개의 페이지에 대한 chunk, block 업데이트
		for(i=0; i<nr_physical_pages; i++){
			ptr_chunk_table[chunk_index+i].valid_count = WRITE_BUFFER_LEN;
			ptr_chunk_table[chunk_index+i].physical_page_len = nr_physical_pages;
			ptr_chunk_table[chunk_index+i].is_compressed = 1;

			if(flash_block->list_pages[page+i].page_status == PAGE_STATUS_FREE){
				flash_block->list_pages[page+i].page_status = PAGE_STATUS_VALID;
				flash_block->list_pages[page+i].no_logical_page_addr = logical_page_address[0];
				flash_block->last_modified_time = timer_get_timestamp_in_us();

				flash_block->nr_valid_pages++;
				flash_block->nr_free_pages--;
			} else {
				goto err;
			}
		}
		...
		if(flash_block->list_pages[page].page_status == PAGE_STATUS_FREE){
			flash_block->list_pages[page].page_status = PAGE_STATUS_VALID;
			flash_block->list_pages[page].no_logical_page_addr = *logical_page_address;
			flash_block->last_modified_time = timer_get_timestamp_in_us();
		...
		ptr_chunk_table[chunk_index].valid_count = 1;
		ptr_chunk_table[chunk_index].physical_page_len = 1;
		ptr_chunk_table[chunk_index].is_compressed = 0;
		
	}


	// for(i=0; i<nr_logical_pages; i++){
	// 	if(logical_page_address[i] < 2){
	// 		printf("- lpa\n");
	// 		printf("lpa %d, compressed %d\n", logical_page_address[i], is_compressed);
	// 		printf("new ppa:chunk_index %d:%d", ptr_pg_mapping->ptr_pg_table[logical_page_address[i]], convert_ppa_to_chunk_index(ptr_pg_mapping->ptr_pg_table[logical_page_address[i]]));
	// 		printf(", validcount%d\n", ptr_chunk_table[convert_ppa_to_chunk_index(ptr_pg_mapping->ptr_pg_table[logical_page_address[i]])].valid_count);
	// 		printf("- end\n\n");
	// 	}
	// }
...
err:
	printf("blueftl_mapping_page:err\n");
	printf("lpa :: ");
	for(i=0; i<nr_logical_pages; i++){ 
		printf("%u, ",logical_page_address[i]);
	}
	printf("\n");
	printf(" the physical page address is already used (%u:%u,%u,%u,%u)\n", ppa, bus, chip, block, page);
	return -1;
}
